...
# calculate week over week percentage change, handling edge cases
def pct_change(current, previous):
    if previous is None or previous == 0 or pd.isna(previous):
        return None
    return (current - previous) / previous

# formatting values for LLM input, handling NaN and datetime
...
llm_input = {
    ...
    "previous_week": to_records(weekly_df.iloc[[1]])[0] if len(weekly_df) > 1 else None,
...
- Write as a finished business report, not as a chatbot response. Do not end with offers, follow-up questions, or phrases like "If you want".
